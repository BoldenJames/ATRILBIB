...
<html>
            ...
            function uRl(variable){
                ...
                $.ajax({
                    ...
                    success: function(res){
                        console.log(res);
                        if(res!='}'){
                            alert('No se puede Generar el archivo .bib (restriccion).');
                        }else{
    ...
    <body>
        ...
        <textarea name="nota" id="nota" cols="30" rows="10"></textarea><button id="lol" onclick='guardarN();return false;'>Guardar</button>
        <?php
            ...
            foreach($titArticulos->find('a[href]') as $link){
                ...
                if($valido === false && $valido1 === false){
                    ...
                    contador($vari);
                    echo "<div class='titulo'><div class='cos' id='art_$vari'>$link<button onclick='uRl($vari)'>URL</button></div><div class='footer'><div class='menu'><button class='left' onclick='showNota();'>Notas</button><button class='right' onclick='real($vari,$idUser);'>Agregar a Libreria</button><button class='right'>Citar</button><input type='hidden' value='$cadena' id='ART_$vari'></div></div></div>";
                }   
            }
